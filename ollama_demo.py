...
def run_demo(transcript, model):
        response = ollama.generate(model=model, prompt=f"Can summarize the given chat transcript in less than 150 words? Chat transcript: {transcript}")
...
response = run_demo(transcript, model)
